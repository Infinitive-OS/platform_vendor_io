...
PRODUCT_VERSION_MINOR = 1.3.1
ifdef IO_BUILD_EXTRA
    IO_POSTFIX := -$(IO_BUILD_EXTRA)
endif
IO_POSTFIX := -$(shell date +"%Y%m%d")
ifeq ($(IO_BUILDTYPE), OFFICIAL)
    IO_BUILD_TYPE := OFFICIAL
else
    IO_BUILD_TYPE := UNOFFICIAL
endif
...
# Set all versions
IO_VERSION := InfinitiveOS-v$(PRODUCT_VERSION_MINOR)-$(IO_BUILD)-$(PRODUCT_VERSION_MAJOR)-$(IO_POSTFIX)-$(IO_BUILD_TYPE)
IO_MOD_VERSION := InfinitiveOS-v$(PRODUCT_VERSION_MINOR)-$(IO_BUILD)-$(PRODUCT_VERSION_MAJOR)-$(IO_POSTFIX)-$(IO_BUILD_TYPE)

PRODUCT_PROPERTY_OVERRIDES += \
    BUILD_DISPLAY_ID=$(BUILD_ID) \
    io.ota.version=$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR) \
